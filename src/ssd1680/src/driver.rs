    ...
    BSY: InputPin,
{
    ...
    pub fn init(
        ...
        // 支持更大高度
        let mux = (self.height) - 1;
        let mux_low = (mux & 0xFF) as u8;
        let mux_high = ((mux >> 8) & 0xFF) as u8;

        self.interface
            .cmd_with_data(cmd::Cmd::DRIVER_CONTROL, &[mux_low, mux_high, 0x00])?;
    ...
    pub fn clear_bw_frame(&mut self) -> Result<(), DisplayError> {
        ...
        self.interface
            .data_x_times(color, u32::from(self.width) / 8 * u32::from(self.height))?;
        Ok(())
    }
    ...
    fn use_full_frame(&mut self) -> Result<(), DisplayError> {
        ...
        self.set_ram_area(0, 0, u32::from(self.width) - 1, u32::from(self.height) - 1)?;
