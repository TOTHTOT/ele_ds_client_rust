...
pub trait Display: DrawTarget {
    ...
    ///
    /// Becomes unnecessary when `const_generics` become stabled
    fn draw_helper(
        &mut self,
        width: u32,
        height: u32,
...
/// Display for an any size panel
pub struct DisplayAnyIn {
    width: u16,
    height: u16,
    buffer: Box<[u8]>,
...
impl DrawTarget for DisplayAnyIn {
        ...
        I: IntoIterator<Item = Pixel<Self::Color>>,
    {
        for p in pixels.into_iter() {
            self.draw_helper(self.width.into(), self.height.into(), p)?;
        }
        Ok(())
    }
}

impl OriginDimensions for DisplayAnyIn {
    fn size(&self) -> Size {
...
#[cfg(test)]
mod tests {
    use super::{find_position, outside_display, Display, DisplayAnyIn, DisplayRotation};
    ...
    #[test]
    fn buffer_clear() {
        let mut display = DisplayAnyIn::bw(128, 296);
