...
- 发送image页面图片资源
  ...
  curl -X PUT --data-binary @test.bmp "http://192.168.137.217/fat/system/images/test.bmp"
  ```
- 发送测试音频页面图片资源
  ```shell
  cd ./assets/tool/audio
  curl -X PUT --data-binary @test_audio.wav "http://192.168.137.217/fat/system/audio/audio.wav"
  ```